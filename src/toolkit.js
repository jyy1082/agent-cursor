 ...
 *
 * Lives in the same repo, and is deployed at the same version tag, as
 * page-pilot/page-pilot-recorder/page-pilot-skills — so it imports them
 * with plain relative paths instead of separately-pinned CDN URLs. This
 * means there's exactly one version number for the whole toolkit to track,
 * and toolkit.js can never end up silently pointing at a mismatched
 * version of one of the other three pieces. The bookmarklet itself still
 * pins THIS repo to a specific version tag when loading toolkit.js (see
 * install.html) — updating still means re-dragging the bookmark, so an
...
if (window.__pagePilotToolkitActive) {
  ...
  init().catch((err) => {
    ...
    alert(
      ...
      'https://github.com/jyy1082/page-pilot'
    );
  });
}
...
async function init() {
  const [{ PagePilot }, { PagePilotRecorder }, Skills] = await Promise.all([
    import('./page-pilot.js'),
    import('./page-pilot-recorder.js'),
    import('./page-pilot-skills.js'),
  ]);
